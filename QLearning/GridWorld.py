...
def action_value_for_tile_type(tile_type):
    """
    static method that creates a dictionary for every possible action in certain state
    :param tile_type: the tile type of the state
    :return: dictionary holding the action-values for each possible action if free tile, otherwise return tile type
    """
    if tile_type == config.TILE_TYPE_FREE:
        action_values = {}
        for action in config.DIRECTIONS:
...
class GridWorld:
    ...
    def create_grids(self):
        ...
        action_value_grid = values depending on action for each state
        ...
        action_value_grid = np.ndarray(shape=np.shape(self.source_grid), dtype=np.object)
        ...
        for i in range(np.shape(self.source_grid)[0]):
            for j in range(np.shape(self.source_grid)[1]):
                ...
                action_value_grid[(i, j)] = action_value_for_tile_type(tile_type)
                policy_grid[(i, j)] = action_for_tile_type(tile_type)
        return value_grid, action_value_grid, policy_grid
    ...
    def side_step(self, position, action):
        """
        returns all possible positions if doing a certain action at current position
        with certain probabilities
        :param position: the state in which the action should be performed
        :param action: the action to be performed in state
        :return: the intended position, sidestep 1, sidestep 2
        """
        forward_step = self.step(position, action)
        if action == config.DIRECTION_NORTH or action == config.DIRECTION_SOUTH:
            side_step1 = self.step(position, config.DIRECTION_EAST)
            side_step2 = self.step(position, config.DIRECTION_WEST)
        elif action == config.DIRECTION_WEST or action == config.DIRECTION_EAST:
            side_step1 = self.step(position, config.DIRECTION_NORTH)
            side_step2 = self.step(position, config.DIRECTION_SOUTH)
        return forward_step, side_step1, side_step2
    ...
    def __init__(self, grid):
        ...
        - action value grid = grid based on source holding current tile values for every possible state action
        ...
        self.value_grid, self.action_value_grid, self.policy_grid = self.create_grids()
